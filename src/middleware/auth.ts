 ...
 *   1. Custom JWT accessToken (cookie: accessToken, or Authorization: Bearer <token>)
 *   2. Better Auth session fallback (cookie: better-auth.session_token)
 * Sets req.user on success.
 */
export const requireAuth = async (req: Request, _res: Response, next: NextFunction) => {
  try {
    // ── Step 1: Custom JWT accessToken (Fast in-memory verification) ─────────
    const rawToken =
    ...
    // ── Step 2: Better Auth session fallback ──────────────────────────────────
